...
function videoHTML(token) {
  ...
  <div class="video-container">
    <video id="thumb2" width="100%" height="100%" preload="metadata" controls>
      <source src="http://localhost:4200/${src}" http://localhost:4200/${dataOrigSrcAttr}>
      <a href="${src}">${src}</a>
    </video>
  </div>`;
}
...
function renderImageOrPlayableMedia(tokens, idx, options, env, slf) {
  ...
  <video id="thumb" controls preload="metadata" width="750px" height="540px">
    source src="http://localhost:4200/uploads/default/original/1X/72308b8d8233f363e0e11a82b0025e63a59c01ae.mp4">
  </video>
  <canvas id="canvas"
          width="750px" height="540px"
          style="display:block;">
